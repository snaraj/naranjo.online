...
func TestMapUsageSumsBothGrammars(t *testing.T) {
	...
	for name, run := range map[string]func() error{
		...
		"overlapping intervals": func() error {
			// Both intervals remain valid inside one UTC day, so only the
			// cross-bucket overlap guard can refuse this document.
			overlap := strings.Replace(split, `"2026-08-09T12:00:00Z"`, `"2026-08-09T06:00:00Z"`, 1)
