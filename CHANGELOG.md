...
- The Art gallery answers three owner findings from a live review (issue
  ...
  blocked reserved 0x0 on Gecko, 194.6x109.4 on Blink and 163x91.7 on WebKit.
...
- The lightbox close control stops sitting on the photograph. The 44px filled
  ...
  an invisible hit box. Measured: the mark's box ends 14px above the framed
  photograph's, on three engines at two viewports. Placing the control
  outside the dialog was measured and rejected — a native `<dialog>` is
  `width: fit-content` with UA `overflow: auto`, and the outside placement
  turned it scrollable (scrollWidth 1194 against clientWidth 1154).
