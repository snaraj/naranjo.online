...
def mutations(facts: PolicyFacts) -> list[tuple[str, Rewriter]]:
    ...
    return [
        ...
        ("policy-egress-tls-rule-loses-its-ports",
         egress(*egress_anchor[:9], *egress_anchor[12:])),
        ("policy-egress-tls-port-widened-to-a-range",
         egress(*egress_anchor[:10],
                "        - port: 1",
                "          endPort: 65535",
                "          protocol: TCP",
                *egress_anchor[12:])),
        ("policy-egress-tls-rule-gains-udp",
         egress(*egress_anchor[:12],
                "        - port: 443",
                "          protocol: UDP",
                *egress_anchor[12:])),
        # The except list mirrors guardedDial's refusal of private and
        # link-local space at the fabric layer. Dropping it whole restores the
        # pre-2026-08-27 rule 1; dropping one entry is the subtler edit -- a
        # narrower except is a WIDER allowance -- and the probe entry is the
        # RFC1918 block the production LAN sits in.
        ("policy-egress-tls-except-list-dropped",
         egress(*egress_anchor[:4], *egress_anchor[9:])),
        ("policy-egress-tls-except-loses-the-lan-block",
         egress(*egress_anchor[:7], *egress_anchor[8:])),
